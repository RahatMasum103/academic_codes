...
void Build_derived_type(int m, int local_m, int n, int local_n,
   ...
   MPI_Type_vector(m, local_n, n, MPI_DOUBLE, block_col_mpi_t_p);

   /* Resize the new type so that it has the extent of local_n doubles */
   //MPI_Type_create_resized(**********);
   MPI_Type_create_resized(MPI_DOUBLE, 0, 2*local_n, block_col_mpi_t_p);
...
void Print_matrix(char title[], double local_A[], int m, int local_n,
   ...
   if (my_rank == 0) {
      ...
      MPI_Gather(local_A, local_n*m, MPI_DOUBLE, A, local_n*m, MPI_DOUBLE, 0, comm);
