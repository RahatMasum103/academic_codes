...
void Read_list(int local_A[], int local_n, int my_rank, int p,
   ...
   //MPI_Scatter(**********);
    ...
    MPI_Scatter(temp, local_n*p, MPI_INT, local_A, local_n*p, MPI_INT, 0, comm);
...
#     endif
      if (my_rank < partner) {
         ...
         MPI_Recv(local_A, sizeof(local_n_p), MPI_INT, partner, 1, comm, &status);
...
#        endif
         ...
         MPI_Send(local_A, sizeof(local_n_p), MPI_INT, partner, 1, comm);
         done = 1;
      }
  }  /* while */
}  /* Sort */
...
void Print_local_lists(int local_A[], int local_n, unsigned bitmask,
   ...
   if (my_rank == 0) {
      ...
      for (q = increment; q < p; q += increment) {
         ...
         MPI_Recv(A, sizeof(A), MPI_INT, q, 0, comm, &status);
         Print_list(A, local_n, q);
      }
      free(A);
   } else {
      MPI_Send(local_A, sizeof(A),  MPI_INT, 0, 0, comm);
   }
}  /* Print_local_lists */
